...
def test_security_posture_snapshot_sanitizes_encryption_provider_error(
    ...
    trustlog_reasons = " ".join(snapshot["trustlog_secure_default"]["reasons"])
    assert "RuntimeError" in trustlog_reasons
    assert "secret path" not in trustlog_reasons
    assert "/prod/foo" not in trustlog_reasons
    assert "token abc" not in trustlog_reasons
...
def test_precomputed_encryption_error_type_non_strict() -> None:
    """Precomputed encryption error_type should produce sanitized degraded diagnostics."""
    result = get_trustlog_security_posture(
        encryption_status={
            "encryption_enabled": False,
            "key_configured": False,
    ...
    assert result["status"] == "degraded"
    assert (
        "TrustLog encryption status retrieval failed: RuntimeError"
        in result["reasons"]
    )
    assert any(
        "VERITAS_ENCRYPTION_KEY_PROVIDER configuration" in remediation
        for remediation in result["remediation"]
    )


def test_precomputed_encryption_error_type_strict() -> None:
    """Precomputed encryption error_type should block strict posture safely."""
    result = get_trustlog_security_posture(
        encryption_status={
            "encryption_enabled": False,
            "key_configured": False,
