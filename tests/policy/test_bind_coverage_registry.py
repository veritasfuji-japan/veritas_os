...
def test_validator_rejects_high_effect_without_fail_closed() -> None:
    result = validate_bind_coverage_registry(
        [_valid_entry(effect_level="high", default_failure_mode="open")]
    )
    assert result.valid is False
...
def test_validator_rejects_human_approval_without_block() -> None:
    result = validate_bind_coverage_registry(
        [
            _valid_entry(
                requires_human_approval=True,
...
def test_validator_requires_implementation_and_docs_refs() -> None:
    result = validate_bind_coverage_registry(
...
def test_validator_rejects_live_integration_claim() -> None:
    result = validate_bind_coverage_registry(
