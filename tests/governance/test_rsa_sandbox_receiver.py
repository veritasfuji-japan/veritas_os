...
@pytest.mark.parametrize("veritas_env", ["production", "stg", " production "])
def test_sandbox_receiver_blocked_in_operational_env_without_enablement(
    monkeypatch: pytest.MonkeyPatch,
    veritas_env: str,
) -> None:
    payload = _payload("SAFE_PROCEED")
...
def test_sandbox_enablement_allows_redacted_execution_in_operational_env(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = _payload("SAFE_PROCEED")
    monkeypatch.setenv("VERITAS_ENV", "production")
...
def test_raw_allow_flag_uses_strip_normalization(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = _payload("SAFE_PROCEED")
    monkeypatch.delenv("VERITAS_ENV", raising=False)
...
def test_raw_output_still_forbidden_in_operational_env_when_enabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = _payload("SAFE_PROCEED")
    monkeypatch.setenv("VERITAS_ENV", "production")
