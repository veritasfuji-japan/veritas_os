...
/**
 * Builds a deterministic React key for timeline rows without using array index.
 */
function buildTimelineKey(item: TrustLogItem): string {
  return [
    String(item.request_id ?? "unknown"),
    String(item.decision_id ?? "unknown"),
    String(item.created_at ?? "na"),
    String(item.stage ?? "unknown"),
    String(item.sha256 ?? "missing"),
    String(item.sha256_prev ?? "missing"),
  ].join("|");
}
