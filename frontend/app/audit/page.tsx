...
import type { RedactionMode, RegulatoryReport } from "./types";
...
export default function TrustLogExplorerPage(): JSX.Element {
  ...
  const [confirmNoRedactionRisk, setConfirmNoRedactionRisk] = useState(false);
  ...
  const periodItemsPreview = useMemo(() => {
  ...
  }, [reportEndDate, reportStartDate, sortedItems]);
  ...
  const createRegulatoryReport = (): RegulatoryReport | null => {
    ...
    if (!Number.isFinite(start) || !Number.isFinite(end) || start > end) {
      setReportError(
        t(
          "期間の指定が不正です。開始日と終了日を確認してください。",
          "Invalid date range. Please verify start and end dates.",
        ),
      );
      return null;
    }
    if (redactionMode === "none" && !confirmNoRedactionRisk) {
      setReportError(
        t(
          "redaction none は高リスクです。追加確認を有効化してください。",
          "Redaction none is high-risk. Please enable additional confirmation.",
        ),
      );
      return null;
    }
    ...
    <div className="space-y-6">
      ...
      <Card title="Selected Audit" titleSize="md" variant="elevated">
        {selected ? (
          <div className="space-y-3 text-xs">
            <div role="tablist" aria-label="Selected audit view" className="flex gap-2">
              <button
                id="selected-audit-summary-tab"
                type="button"
                role="tab"
                aria-selected={selectedTab === "summary"}
                aria-controls="selected-audit-summary-panel"
              ...
              <button
                id="selected-audit-raw-tab"
                type="button"
                role="tab"
                aria-selected={selectedTab === "raw"}
                aria-controls="selected-audit-raw-panel"
            ...
            {selectedTab === "summary" ? (
              <div
                id="selected-audit-summary-panel"
                role="tabpanel"
                aria-labelledby="selected-audit-summary-tab"
              >
              ...
              <div
                id="selected-audit-raw-panel"
                role="tabpanel"
                aria-labelledby="selected-audit-raw-tab"
              >
      ...
      <Card title={t("TrustLog インタラクティブ検証", "TrustLog Interactive Verification")} titleSize="md" variant="elevated" accent="success">
        <div className="space-y-3">
          ...
          {verificationMessage ? <p role="status" className="text-xs">{verificationMessage}</p> : null}
        ...
        <div className="space-y-3 text-sm">
          ...
          <label className="text-xs">
            ...
            <select
              ...
              value={redactionMode}
              onChange={(event) => {
                const mode = event.target.value as RedactionMode;
                setRedactionMode(mode);
                if (mode !== "none") {
                  setConfirmNoRedactionRisk(false);
                }
              }}
            >
          ...
          {redactionMode === "none" ? (
            <label className="flex items-start gap-2 text-xs text-warning">
              <input
                type="checkbox"
                checked={confirmNoRedactionRisk}
                onChange={(event) => setConfirmNoRedactionRisk(event.target.checked)}
              />
              <span>
                {t(
                  "none モードはPII漏えいリスクが高いことを理解しました。",
                  "I understand that none mode carries elevated PII leakage risk.",
                )}
              </span>
            </label>
          ) : null}
          <p className="text-xs">Export preview: {periodItemsPreview} entries in selected period.</p>
          <div className="flex gap-2">
