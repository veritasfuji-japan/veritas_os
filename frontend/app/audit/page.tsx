...
export default function TrustLogExplorerPage(): JSX.Element {
  ...
  const exportPreviewCount = useMemo(
    () => countEntriesInPeriod(sortedItems, reportStartDate, reportEndDate),
    [reportEndDate, reportStartDate, sortedItems],
  );

  const allowUnsafeRedaction = process.env.NODE_ENV !== "production";
        ...
        <div className="space-y-3 text-sm">
          ...
          <label className="text-xs">
            ...
            <select aria-label="redaction mode" className="ml-2 rounded border border-border px-2 py-1" value={redactionMode} onChange={(event) => setRedactionMode(event.target.value)}>
              ...
              {allowUnsafeRedaction ? <option value="none">none (internal only)</option> : null}
            </select>
          </label>
          {!allowUnsafeRedaction ? <p className="text-xs text-warning">Security mode: redaction=none is disabled in production builds.</p> : null}
          <p className="text-xs">Export preview: {exportPreviewCount} entries in selected period.</p>
          <div className="flex gap-2">
