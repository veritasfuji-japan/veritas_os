...
it("rejects non-minimal operator_verbosity enum", () => {
  const validation = validateGovernancePolicyResponse({
    ok: true,
    policy: {
      ...validGovernanceResponse.policy,
      operator_verbosity: "verbose",
    },
  });
  ...
  expect(validation.issues.some((issue) => issue.path === "policy.operator_verbosity")).toBe(true);
});
