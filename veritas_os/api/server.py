...
GOVERNANCE_STORE = GovernancePolicyStore(GOVERNANCE_POLICY_PATH)
...
def get_governance_policy() -> dict:
    """Return current governance policy from persistent store."""
    policy = GOVERNANCE_STORE.load_policy()
...
def put_governance_policy(body: GovernancePolicyUpdateRequest) -> dict:
    """Persist governance policy and return before/after for diff preview support."""
    before = GOVERNANCE_STORE.load_policy()
    after = GOVERNANCE_STORE.save_policy(body.policy)
    _publish_event(
        "governance.policy.updated",
        {"before": before.model_dump(), "after": after.model_dump()},
    )
    return {"ok": True, "before": before.model_dump(), "policy": after.model_dump()}
