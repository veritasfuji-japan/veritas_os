...
def _bind_failure_status(bind_receipt: Dict[str, Any]) -> int:
    """Map bind failure details to legacy-compatible HTTP status codes."""
    if bind_receipt.get("final_outcome") != FinalOutcome.APPLY_FAILED.value:
        return 500
    rollback_reason = str(bind_receipt.get("rollback_reason") or "")
    if "BIND_INPUT_VALIDATION:" in rollback_reason:
        return 400
    return 500
...
def governance_put(body: dict):
    ...
    try:
        ...
        bind_receipt = update_governance_policy_with_bind_boundary(
            ...
            decision_hash=sha256_of_canonical_json(body),
            policy_patch=body,
        ...
        if bind_receipt.get("final_outcome") != FinalOutcome.COMMITTED.value:
            failure_status = _bind_failure_status(bind_receipt)
            error_message = (
                "Governance policy validation failed"
                if failure_status == 400
                else "Failed to update governance policy"
            )
            return JSONResponse(
                ...
                content={
                    ...
                    "error": error_message,
                    "policy": updated,
                    **{key: value for key, value in bind_payload.items() if key != "ok"},
                },
            )
        srv._publish_event(
            "governance.updated",
