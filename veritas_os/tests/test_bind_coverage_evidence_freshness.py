...
def test_missing_generated_at_in_generated_json_is_stale(tmp_path: Path) -> None:
    ...
    stale_files, _ = compare_bind_coverage_evidence(
...
def test_regenerate_command_uses_module_execution() -> None:
    """Regenerate command should use module execution for stable imports."""
    assert REGENERATE_COMMAND == "python -m scripts.governance.export_bind_coverage_evidence"


def test_missing_generated_at_reason_is_printed(tmp_path: Path, capsys) -> None:
    """Missing generated_at should print actionable reason without traceback."""
    committed_json, committed_md = _write_committed_artifacts(tmp_path, FIXED_GENERATED_AT)
    committed_json.write_text('{"schema_version":"bind_coverage_evidence.v1"}\n', encoding="utf-8")
    ...
    assert result == 1
    assert "missing generated_at" in output
    assert "Traceback" not in output


def test_invalid_generated_at_reason_is_printed(tmp_path: Path, capsys) -> None:
    """Invalid generated_at should print actionable reason without traceback."""
    committed_json, committed_md = _write_committed_artifacts(tmp_path, FIXED_GENERATED_AT)
...
def test_makefile_and_ci_use_module_execution() -> None:
    ...
    workflow_text = (repo_root / ".github/workflows/main.yml").read_text(encoding="utf-8")

    assert "python -m scripts.governance.export_bind_coverage_evidence" in makefile_text
    assert "python -m scripts.governance.check_bind_coverage_evidence_freshness" in makefile_text
    assert "python -m scripts.governance.check_bind_coverage_evidence_freshness" in workflow_text
