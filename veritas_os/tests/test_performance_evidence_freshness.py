...
def test_generated_at_non_string_is_stale(tmp_path: Path) -> None:
    committed_json, committed_md = _write_fresh_artifacts(tmp_path)
    payload = json.loads(committed_json.read_text(encoding="utf-8"))
...
def test_generated_at_blank_is_stale(tmp_path: Path) -> None:
    committed_json, committed_md = _write_fresh_artifacts(tmp_path)
    payload = json.loads(committed_json.read_text(encoding="utf-8"))
...
def test_generated_at_invalid_iso_is_stale(tmp_path: Path) -> None:
    committed_json, committed_md = _write_fresh_artifacts(tmp_path)
    payload = json.loads(committed_json.read_text(encoding="utf-8"))
...
def test_measurement_mode_non_string_is_stale(tmp_path: Path) -> None:
    committed_json, committed_md = _write_fresh_artifacts(tmp_path)
    payload = json.loads(committed_json.read_text(encoding="utf-8"))
...
def test_measurement_mode_outside_allowed_is_stale(tmp_path: Path) -> None:
    committed_json, committed_md = _write_fresh_artifacts(tmp_path)
    payload = json.loads(committed_json.read_text(encoding="utf-8"))
...
def test_missing_required_field_does_not_add_generic_json_mismatch(
    ...
    assert result.fresh is False
    assert any("missing required field metrics" in reason for reason in result.reasons)
    assert not any("metrics must be a list" in reason for reason in result.reasons)
    assert not any("JSON payload mismatch" in reason for reason in result.reasons)
