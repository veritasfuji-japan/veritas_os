...
class TestStrictPostureEncryptionBackend:
    ...
    def test_require_production_flag_enforces_strict_backend_in_dev_posture(
        ...
        with pytest.raises(EncryptionBackendUnavailable) as encrypt_exc:
            encrypt("strict due to enforcement flag")
        assert "VERITAS_REQUIRE_PRODUCTION_TRUSTLOG_POSTURE" in str(encrypt_exc.value)

        with pytest.raises(EncryptionBackendUnavailable) as excinfo:
            decrypt("ENC:hmac-ctr:Zm9v")
        decrypt_message = str(excinfo.value)
        assert "VERITAS_ENCRYPTION_LEGACY_DECRYPT" in decrypt_message
        assert (
            "HMAC-CTR" in decrypt_message
            or "hmac-ctr" in decrypt_message.lower()
        )
