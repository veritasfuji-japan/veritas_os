...
def test_requests_compat_rejects_non_http_schemes() -> None:
    """urllib fallback must reject file/custom schemes before opening them."""
    from veritas_os.tools._requests_compat import RequestException
    from veritas_os.tools._requests_compat import _RequestsCompat

    compat = _RequestsCompat()

    with pytest.raises(RequestException, match="Unsupported URL scheme"):
        compat.get("file:///tmp/test.txt")
