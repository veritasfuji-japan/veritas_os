...
def test_secure_posture_alias_enforces_production_posture() -> None:
    result = check_trustlog_production_posture({"VERITAS_POSTURE": "secure"})
    assert result.passed is False
    assert any("backend must be postgresql" in item for item in result.failures)


def test_hardened_posture_alias_enforces_production_posture() -> None:
    result = check_trustlog_production_posture({"VERITAS_POSTURE": "hardened"})
    assert result.passed is False
    assert any("backend must be postgresql" in item for item in result.failures)


def test_prod_posture_alias_enforces_production_posture() -> None:
    result = check_trustlog_production_posture({"VERITAS_POSTURE": "prod"})
    assert result.passed is False
    assert any("backend must be postgresql" in item for item in result.failures)


def test_production_posture_alias_enforces_production_posture() -> None:
    result = check_trustlog_production_posture({"VERITAS_POSTURE": "production"})
    assert result.passed is False
    assert any("backend must be postgresql" in item for item in result.failures)


def test_local_env_without_posture_is_not_enforced() -> None:
