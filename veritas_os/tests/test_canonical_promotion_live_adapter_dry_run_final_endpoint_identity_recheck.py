...
def _candidate(source=None, **changes):
    source = source if source is not None else SOURCE_PACKET
    value = dict(source.endpoint_candidate)
    value.update(changes)
    return value


def _packet(*, source=None, candidate=None, rechecked_at=RECHECKED_AT):
    source = source if source is not None else SOURCE_PACKET
...
def test_result_truthfully_limits_recheck_to_local_metadata():
    result = _packet().final_endpoint_identity_recheck_result
...
def test_recheck_timestamp_changes_packet_not_frozen_bind_or_endpoint_identity():
    first = _packet()
    later = _packet(rechecked_at=RECHECKED_AT + timedelta(seconds=1))

    assert first.bind_context_hash == later.bind_context_hash
    assert first.rechecked_endpoint_candidate_digest == later.rechecked_endpoint_candidate_digest
    assert (
    ...
    assert (
        ...
        != later.promotion_live_adapter_dry_run_final_endpoint_identity_recheck_hash
    )
...
def test_any_endpoint_identity_change_fails_closed(field, value):
    with pytest.raises(
        CanonicalPromotionLiveAdapterDryRunFinalEndpointIdentityRecheckError
    ):
        _packet(candidate=_candidate(**{field: value}))


def test_sensitive_or_open_endpoint_input_fails_closed():
    candidate = _candidate()
    candidate["token"] = "must-not-be-accepted"
    with pytest.raises(
        CanonicalPromotionLiveAdapterDryRunFinalEndpointIdentityRecheckError
    ):
        _packet(candidate=candidate)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("bind_context_hash_derivation_state",), "tampered"),
        ...
        (("exact_bind_context", "endpoint_candidate_digest"), "0" * 64),
        (("exact_bind_context", "endpoint_identity_binding_digest"), "0" * 64),
...
def test_representative_source_effect_or_authority_claim_fails_closed(field):
    with pytest.raises(
        CanonicalPromotionLiveAdapterDryRunFinalEndpointIdentityRecheckError
    ):
...
def test_invalid_or_naive_recheck_timestamp_fails_closed(rechecked_at):
    with pytest.raises(
        CanonicalPromotionLiveAdapterDryRunFinalEndpointIdentityRecheckError
    ):
        _packet(rechecked_at=rechecked_at)


@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("rechecked_endpoint_candidate_digest", "0" * 64),
        ("final_endpoint_identity_binding_digest", "0" * 64),
        ("final_endpoint_identity_recheck_result_digest", "0" * 64),
        ("final_endpoint_identity_recheck_context_digest", "0" * 64),
        ("final_endpoint_identity_recheck_check_digest", "0" * 64),
        ("final_endpoint_identity_rechecked", False),
...
def test_nested_binding_candidate_or_result_tamper_fails_reconstruction():
    raw = _packet().model_dump(mode="json")
    ...
    raw = _packet().model_dump(mode="json")
    raw["rechecked_endpoint_candidate"]["endpoint_host"] = "other.invalid"
    with pytest.raises(
        CanonicalPromotionLiveAdapterDryRunFinalEndpointIdentityRecheckError
    ):
        verify_canonical_promotion_live_adapter_dry_run_final_endpoint_identity_recheck_packet(
