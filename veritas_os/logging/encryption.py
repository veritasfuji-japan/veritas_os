...
def _require_strong_encryption_backend() -> None:
    ...
    if _USE_REAL_AES:
        return
    if not _strict_encryption_backend_required():
        return
    posture = resolve_posture()
...
def is_strong_encryption_backend_available() -> bool:
    """Return whether cryptography-backed AES-256-GCM is available."""
    return bool(_USE_REAL_AES)
