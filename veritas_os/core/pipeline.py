...
NOTE: This file was split for maintainability.  Sub-modules:
  pipeline_helpers.py   — constants, pure utilities, core-decide wrapper
  pipeline_memory.py    — MemoryOS adapter
  pipeline_web.py       — WebSearch adapter
  pipeline_evidence.py  — evidence normalization / extraction
  pipeline_critique.py  — critique enforcement (ISSUE-2)
  pipeline_contract.py  — contract enforcement helpers
"""
...
# =========================================================
# Tests & server access symbols via ``pipeline.xxx``.
# Every public name that existed before the split MUST remain
...
# --- pipeline_helpers (constants + utils + core-decide) ---
...
# =========================================================
# Safe imports (core modules) — kept here for backward compat
# =========================================================
# Tests access these as  pipeline.veritas_core / pipeline.mem  etc.
...
# =========================================================
# evidence_core (for step1 hardening)
# =========================================================
from veritas_os.core import evidence as evidence_core


# =========================================================
# web_search function (patchable by tests via pipeline.web_search)
# =========================================================
try:
...
async def _safe_web_search(query: str, *, max_results: int = 5):  # noqa: F811
    """Wrapper that delegates to module-level ``web_search`` so tests can monkeypatch it."""
    import inspect as _inspect
    fn = web_search
    if not callable(fn):
        return None
    try:
        ws = fn(query, max_results=max_results)
        if _inspect.isawaitable(ws):
            ws = await ws
...
# Override _load_valstats / _save_valstats so monkeypatching pipeline.VAL_JSON works
def _load_valstats() -> Dict[str, Any]:  # noqa: F811
    try:
        p = Path(VAL_JSON)
        if p.exists():
...
def _save_valstats(d: Dict[str, Any]) -> None:  # noqa: F811
    try:
        p = Path(VAL_JSON)
...
# Override _allow_prob so monkeypatching pipeline.predict_gate_label works
def _allow_prob(text: str) -> float:  # noqa: F811
...
async def run_decide_pipeline(
    ...
    _check_required_modules(veritas_core, fuji_core)
    ...
    _world_model = _lazy_import("veritas_os.core.world", None) or _lazy_import("veritas_os.core.world_model", None)
    try:
        if _world_model is not None and hasattr(_world_model, "inject_state_into_context"):
            context = _world_model.inject_state_into_context(context, user_id)  # type: ignore
    ...
    memory_store = _get_memory_store(mem)
    ...
    # =========================================================
    # WebSearch (optional / best-effort) + contract
    # =========================================================
    ...
    # =========================================================
    _veritas_core = _lazy_import("veritas_os.core.kernel", None) or _lazy_import("veritas_os.core", "kernel")
    _debate_core = _lazy_import("veritas_os.core.debate", None) or _lazy_import("veritas_os.core", "debate")
    _reason_core = _lazy_import("veritas_os.core.reason", None) or _lazy_import("veritas_os.core", "reason")
    _fuji_core = _lazy_import("veritas_os.core.fuji", None) or _lazy_import("veritas_os.core", "fuji")
    _value_core = _lazy_import("veritas_os.core.value_core", None) or _lazy_import("veritas_os.core", "value_core")

    core_decide = None
    try:
        if _veritas_core is not None and hasattr(_veritas_core, "decide"):
            core_decide = _veritas_core.decide  # type: ignore[attr-defined]
    except Exception:
        core_decide = None
    ...
    if raw and healing_enabled:
        ...
        while True:
            ...
            budget_snapshot = self_healing.budget_remaining(healing_state, healing_budget)
    ...
    alts = _dedupe_alts(alts, _veritas_core)

    # =========================================================
    # WorldModel boost (best-effort)
    # =========================================================
    try:
        if _world_model is not None and hasattr(_world_model, "simulate"):
            boosted: List[Dict[str, Any]] = []
            uid_for_world = str((context or {}).get("user_id") or user_id or "anon")

            for d in alts:
                sim = _world_model.simulate(user_id=uid_for_world, query=query, chosen=d)  # type: ignore
    ...
    try:
        if _debate_core is not None and hasattr(_debate_core, "run_debate") and not fast_mode:
            debate_result = _debate_core.run_debate(
    ...
    # =========================================================
    try:
        if _fuji_core is not None and hasattr(_fuji_core, "validate_action"):
            fuji_pre = _fuji_core.validate_action(query, context)  # type: ignore
        elif _fuji_core is not None and hasattr(_fuji_core, "validate"):
            fuji_pre = _fuji_core.validate(query, context)  # type: ignore
        else:
    ...
    status_map = {"ok": "allow", "allow": "allow", "pass": "allow", "modify": "modify", "block": "rejected", "deny": "rejected", "rejected": "rejected"}
    try:
    ...
    fuji_dict = {**(fuji_dict if isinstance(fuji_dict, dict) else {}), **(fuji_pre if isinstance(fuji_pre, dict) else {})}
    ...
    ev_fuji = _norm_evidence_item({
        "source": "internal:fuji", "uri": None,
        "snippet": f"[FUJI pre] status={fuji_status}, risk={risk_val}, reasons={'; '.join(reasons_list) if reasons_list else '-'}, violations={', '.join(viols) if viols else '-'}",
    ...
    # =========================================================
    try:
        if _value_core is not None and hasattr(_value_core, "evaluate"):
            vc = _value_core.evaluate(query, context or {})  # type: ignore
            values_payload = {
    ...
    try:
        ...
        for d in alts:
            ...
            util = base * (0.5 + 0.5 * v_total) * (1.0 - r_val) * (0.5 + 0.5 * t_val)
            util = _clip01(util)
            d.setdefault("world", {})
    ...
    mem_evi_cnt = sum(1 for ev in evidence if isinstance(ev, dict) and str(ev.get("source", "")).startswith("memory"))
    ...
    # =========================================================
    # Evidence normalize/dedupe/cap
    # =========================================================
    evidence = [ev for ev in (_norm_evidence_item(x) for x in evidence) if ev]
    ...
    res: Dict[str, Any] = {
        ...
        "chosen": chosen, "alternatives": alts, "options": list(alts),
        "evidence": evidence, "critique": critique, "debate": debate,
        "telos_score": float(telos), "fuji": fuji_dict, "extras": response_extras,
        "gate": {"risk": float(effective_risk), "telos_score": float(telos), "decision_status": decision_status, "reason": rejection_reason, "modifications": modifications},
        "values": values_payload,
        "persona": load_persona(),
        "version": os.getenv("VERITAS_API_VERSION", "veritas-api 1.x"),
        "decision_status": decision_status, "rejection_reason": rejection_reason,
    ...
    # =========================================================
    try:
        audit_entry = redact_payload({
            "request_id": request_id, "created_at": utc_now().isoformat(),
            "context": context, "query": query, "chosen": chosen,
    ...
    # =========================================================
    try:
        store2 = _get_memory_store(mem)
        if store2 is not None:
            decision_key = f"decision_{request_id}"
            decision_value = redact_payload({
                "kind": "decision", "request_id": request_id, "query": query,
                "chosen": payload.get("chosen"), "gate": payload.get("gate"),
                "values": payload.get("values"), "extras": payload.get("extras"),
                "created_at": utc_now_iso_z(),
            })
            _memory_put(store2, user_id, key=decision_key, value=decision_value)

            episode_key = f"episode_{time.time_ns()}_{request_id[:8]}"
            episode_value = redact_payload({
                "kind": "episode", "request_id": request_id, "query": query,
                "chosen": payload.get("chosen"),
                "decision_status": payload.get("decision_status"),
                "rejection_reason": payload.get("rejection_reason"),
                "created_at": utc_now_iso_z(),
            })
            _memory_put(store2, user_id, key=episode_key, value=episode_value)
    except Exception as e:
    ...
    # =========================================================
    try:
        if _reason_core is not None and hasattr(_reason_core, "reflect"):
            reflection = _reason_core.reflect({
                "query": query, "chosen": payload.get("chosen", {}),
                "gate": payload.get("gate", {}), "values": payload.get("values", {}),
            })
        else:
        ...
        try:
            ...
            entry = {
                "created_at": utc_now_iso_z(), "request_id": request_id,
                "next_value_boost": float(reflection.get("next_value_boost", 0.0) or 0.0),
                "value_ema": ema2, "source": "reason_core", "fast_mode": bool(fast_mode),
            }
        ...
        try:
            llm_reason = None
            if _reason_core is not None and hasattr(_reason_core, "generate_reason"):
                llm_reason = _reason_core.generate_reason(
                    query=query, planner=payload.get("planner") or payload.get("plan"),
                    values=payload.get("values"), gate=payload.get("gate"), context=context,
                )
            ...
            payload["reason"] = {
                "note": note_text, "next_value_boost": reflection.get("next_value_boost", 0.0),
                "reflection": reflection, "llm": llm_reason,
            }
        except Exception as e2:
    ...
    try:
        ...
        meta_ds = {
            ...
            "latency_ms": duration_ms_ds, "fast_mode": bool(fast_mode),
        }
    ...
    try:
        ...
        if len(payload_evidence) == 0:
            payload_evidence = list(evidence) if isinstance(evidence, list) and len(evidence) > 0 else []
    ...
    try:
        ...
        evidence_list: List[Dict[str, Any]] = [ev for ev in (payload.get("evidence") or []) if isinstance(ev, dict)]

        mem_evidence_count2 = sum(1 for ev in evidence_list if str(ev.get("source", "")).lower().startswith("memory"))

        meta_payload = payload.get("meta") or {}
        if not isinstance(meta_payload, dict):
            meta_payload = {}

        meta_payload["memory_evidence_count"] = int(metrics2.get("memory_evidence_count", 0) or 0)
        ...
        persist = {
            "request_id": request_id, "ts": utc_now_iso_z(timespec="seconds"),
            "query": query, "chosen": payload.get("chosen"),
            ...
            "fuji": fuji_full, "latency_ms": latency_ms2,
            ...
            "context": context, "world": world_snapshot,
    ...
    # =========================================================
    try:
        if _world_model is not None and hasattr(_world_model, "update_from_decision"):
            uid_world = str((context or {}).get("user_id") or user_id or "anon")
            ...
            _world_model.update_from_decision(
                user_id=uid_world,
    ...
    try:
        if _world_model is not None and hasattr(_world_model, "next_hint_for_veritas_agi"):
            agi_info = _world_model.next_hint_for_veritas_agi()  # type: ignore
