...
def resolve_decision_precedence(
    *values: object,
    ...
    A permissive source cannot override a restrictive source. Unknown,
    malformed, blank, or absent values fail closed to the reject/block severity.
    When ``output`` names a VERITAS surface, the returned term is canonical for
    that surface. With ``output="preserve"``, recognized normalized decision
    terms are preserved where possible; raw input strings are not preserved, and
    unsupported values resolve to ``"block"``.
    """
    if not values:
        severity = 3
        selected = "unknown"
