...
def reload_policy() -> Dict[str, Any]:
    """Reload the shared FUJI policy module and keep this module alias synced."""
    global POLICY
...
def _load_policy(path: Path | None) -> Dict[str, Any]:
    """Backward-compatible wrapper around the shared FUJI policy loader."""
    _fuji_policy.yaml = yaml
...
def _load_policy_from_str(content: str, path: Path) -> Dict[str, Any]:
    """Backward-compatible wrapper around the shared string policy loader."""
    _fuji_policy.yaml = yaml
