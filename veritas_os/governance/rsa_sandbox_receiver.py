...
def _reason_code_for_status(rsa_status: str) -> str:
    try:
        return _RSA_STATUS_CONTRACTS[rsa_status].reason_code
...
def evaluate_rsa_sandbox_signal(
    ...
    if include_raw_upstream_fields:
        veritas_env = os.getenv("VERITAS_ENV", "").lower()
        if veritas_env in {"prod", "production", "staging"}:
            raise ValueError(
    ...
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise ValueError(
            "RSASandboxPayload.timestamp must include timezone information"
        )

    decision = _build_veritas_decision(payload.rsa_status)
    veritas_reason = _veritas_reason_for_status(payload.rsa_status)
    original_llm_intent = (
