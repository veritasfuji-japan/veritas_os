...
_OPERATIONAL_ENVIRONMENTS = {"prod", "production", "stg", "staging"}
...
def evaluate_rsa_sandbox_signal(
    ...
    for field_name, value in (
        ...
        if not isinstance(value, str) or not value.strip():
            raise ValueError(f"RSASandboxPayload.{field_name} must be a non-empty string")

    if include_raw_upstream_fields:
        veritas_env = os.getenv("VERITAS_ENV", "").strip().lower()
        if veritas_env in _OPERATIONAL_ENVIRONMENTS:
            raise ValueError(
                "Raw upstream RSA sandbox fields are not allowed in operational environments"
            )
        allow_raw_upstream = (
            os.getenv("VERITAS_RSA_SANDBOX_ALLOW_RAW_UPSTREAM", "").strip() == "1"
        )
        if not allow_raw_upstream:
            raise ValueError(
    ...
    veritas_env = os.getenv("VERITAS_ENV", "").strip().lower()
    sandbox_enabled = os.getenv("VERITAS_RSA_SANDBOX_ENABLE", "").strip() == "1"
    if veritas_env in _OPERATIONAL_ENVIRONMENTS and not sandbox_enabled:
        raise ValueError(
            "RSA sandbox receiver is not allowed in operational environments "
            "without VERITAS_RSA_SANDBOX_ENABLE=1"
        )
