"""Shared pure normalization helpers for TrustLog backend aliases."""

from __future__ import annotations


def _normalize_raw(raw: str | None) -> str:
    """Return stripped lowercase backend token."""
    return (raw or "").strip().lower()


def normalize_trustlog_signer_backend(raw: str | None) -> str:
    """Normalize TrustLog signer backend aliases."""
...
def normalize_trustlog_mirror_backend(raw: str | None) -> str:
    """Normalize TrustLog mirror backend aliases."""
    normalized = _normalize_raw(raw)
...
def normalize_trustlog_anchor_backend(raw: str | None) -> str:
    """Normalize TrustLog anchor backend aliases."""
    normalized = _normalize_raw(raw)
