...
戻り値フォーマット:
{
  ...
  "meta": {
     ...
     "toxicity_filter_applied": bool,    # ★入力毒性フィルタ適用有無
     "toxicity_blocked_count": int,      # 毒性判定で弾いた件数
  }
}
...
# 外部検索結果に混入しうる retrieval poisoning / prompt injection パターン。
TOXICITY_PATTERNS: tuple[re.Pattern[str], ...] = (
...
def _build_meta(
    *,
    ...
    blocked_count: int,
    toxicity_filter_applied: bool,
    toxicity_blocked_count: int,
) -> Dict[str, Any]:
    ...
    return {
        ...
        "toxicity_filter_applied": toxicity_filter_applied,
        "toxicity_blocked_count": toxicity_blocked_count,
    }


def _is_toxic_result(title: str, snippet: str, url: str) -> bool:
    """Return True when a search hit looks like retrieval poisoning content.
...
def web_search(query: str, max_results: int = 5) -> Dict[str, Any]:
    ...
    if not raw_query:
        return {
            ...
            "meta": _build_meta(
                ...
                blacklist_applied=False,
                blocked_count=0,
                toxicity_filter_applied=False,
                toxicity_blocked_count=0,
            ),
        }
    ...
    try:
        ...
        # ----------------------------
        # 4) 外部検索スニペットの毒性フィルタ（RAG poisoning軽減）
        # ----------------------------
        toxicity_filter_enabled = (
            os.getenv("VERITAS_WEBSEARCH_ENABLE_TOXICITY_FILTER", "1")
            .strip()
            .lower()
            ...
            if not agi_items:
                return {
                    ...
                    "meta": _build_meta(
                        ...
                        blacklist_applied=blacklist_applied,
                        blocked_count=blocked_count,
                        toxicity_filter_applied=toxicity_filter_enabled,
                        toxicity_blocked_count=toxicity_blocked_count,
                    ),
                }
