...
> **PoC 入口の俯瞰** — 本 README には 3 種類の PoC サーフェスがあります:
> 1. **One-Day PoC 証跡パック** (冒頭の冒頭ブロックで案内): 稼働中 VERITAS API に対する smoke スクリプト + サニタイズ済み証跡パック。
> 2. **AML/KYC レビュアー向けウォークスルー Quickstart**（このセクション）: `/?demo_scenario=aml_kyc_reviewer_walkthrough` でアクセスする決定論的 fixture デモ。
> 3. [AML/KYC Beachhead PoC Pack](#amlkyc-beachhead-poc-pack1日で試せる内容): `scripts/run_aml_kyc_poc_fixture.py` から実行する 1-day fixture セット。
>
> 一般レビュアー証跡なら #1、UI ウォークスルーなら #2、regulated-action fixture セットなら #3 を起点としてください。
...
> 以下の図は Mermaid 対応ビューア（GitHub、GitLab、近代的な Markdown プレビュー）でレンダリングされます。`flowchart LR` のソースが生のまま表示される場合は、図の直後にある番号付き散文の説明を参照してください — どちらも同じワークフローを表現しています。
...
## ⚡ 60秒スタート（Docker Compose TL;DR）

> ローカル開発・Docker の詳細手順は下の [Quick Start](#-quick-start) を参照してください。
...
- [60秒スタート（Docker Compose TL;DR）](#-60秒スタートdocker-compose-tldr)
...
パイプラインステージ（17 トレースステージ — ID は `veritas_os/core/pipeline/__init__.py` の `trace_session.stage(...)` と一致）:
...
> FUJI / ValueCore / リプレイスナップショット構築の各サブ処理は、親ステージ
> （`fuji_gate` / `value_learning_ema` / `persist`）の内部で実行されます。
> 監査永続化、メモリ書込み、世界状態更新、リプレイスナップショット構築は
> 単一の `persist` ステージにまとめられます。
...
│   │   ├── server.py        ← 48+エンドポイントを持つFastAPIアプリ
...
│   └── tests/               ← 7800以上のPythonテスト（+ トップレベルtests/）
...
完全な手順（ロールバック含む）は[`docs/ja/operations/postgresql-production-guide.md` §11](docs/ja/operations/postgresql-production-guide.md)を参照してください。
...
[`docs/ja/operations/postgresql-production-guide.md`](docs/ja/operations/postgresql-production-guide.md) を参照:
...
[`docs/ja/operations/postgresql-drill-runbook.md`](docs/ja/operations/postgresql-drill-runbook.md) を参照:
...
運用ガイドと結果解釈は
[`docs/ja/guides/governance-policy-bundle-promotion.md`](docs/ja/guides/governance-policy-bundle-promotion.md)
を参照してください。
...
> ライフサイクル全体、鍵管理、移行ガイドは [`docs/ja/governance/governance-artifact-lifecycle.md`](docs/ja/governance/governance-artifact-lifecycle.md) を参照してください。
...
# バックエンド/フロントエンドのターミナルでは同じ .env を source してください。
set -a && source .env && set +a
pnpm ui:dev
```
...
│  ┌────────────────────────────────────────────────┐  │
│  │           API サーバー (server.py)             │  │
│  │     認証 · レート制限 · CORS · PII マスク      │  │
│  └────────────────────────────────────────────────┘  │
│  ┌──────────┬──────────┬────────┬───────┬────────┐   │
│  │ Pipeline │ガバナンス│ Memory │ Trust │ Compl- │   │
│  │  Orchestr│   API    │  API   │  API  │ iance  │   │
│  └──────────┴──────────┴────────┴───────┴────────┘   │
│  ┌────────────────────────────────────────────────┐  │
│  │           コア意思決定エンジン                  │  │
│  │  ┌────────┐┌────────┐┌────────┐┌─────────┐    │  │
...
│  ┌────────────────────────────────────────────────┐  │
│  │  インフラストラクチャ                          │  │
│  │  LLM クライアント · TrustLog · Replay · 無害化 │  │
...
完全な段階モデルは [`docs/ja/validation/production-validation.md`](docs/ja/validation/production-validation.md)、
...
[`docs/ja/validation/production-validation.md`](docs/ja/validation/production-validation.md) を参照してください。
...
> `year` フィールドは引用対象 DOI の Zenodo 預入年（安定参照スナップショット）を
> 表します。README 内の他の日付はプロジェクト進捗（現行ベータは 2026 年運用）を
> 指しており、引用年と意図的に区別されます。
