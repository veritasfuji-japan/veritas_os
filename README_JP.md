...
Reviewer Evidence Packet Validation workflow では、生成された validation report、generated packet、golden fixture、schema を GitHub Actions artifact としてアップロードします。これにより、レビュアーは CI 上で生成・検証された成果物を直接確認できます。アップロードされる CI artifact には `reviewer-evidence-artifact-manifest.json` も含まれます。これは、各 reviewer artifact の役割、source、sha256 hash、size を記録した deterministic manifest です。
