...
VERITAS OS は、LLM（例: OpenAI GPT-4.1-mini）を **決定論的・安全ゲート付き・ハッシュチェーン監査可能** な意思決定パイプラインで包み、リアルタイム運用可視化のための **Mission Controlダッシュボード**（Next.js）を提供します。
...
- 最終ゲート（**FUJI Gate**）でPII検出・有害コンテンツブロック・プロンプトインジェクション防御・ポリシー駆動ルールによる安全性とコンプライアンスを強制
- 再現可能な意思決定パイプライン（20以上のステージ、構造化出力、決定論的リプレイ）
- **ハッシュチェーンTrustLog** による監査可能性（改ざん検知、Ed25519署名、WORMミラー対応）
...
```text
入力 → 正規化 → フォールバック代替案 → メモリ検索 → Web検索
  → コア実行 → モデルブースト → 討論 → 批評 → 価値学習（EMA）
  → メトリクス計算 → エビデンス強化 → FUJIプレチェック → ValueCore
  → ゲート判定 → レスポンス組立 → エビデンス最終化
  → 監査ログ永続化 → メモリ永続化 → 世界状態永続化 → リプレイスナップショット構築
```
...
| PUT | `/v1/governance/policy` | ガバナンスポリシー更新（ホットリロード） |
...
| モジュール | 責務 |
|---|---|
| `veritas_os/core/fuji.py` | 多層安全ゲート — PII、有害コンテンツ、機微ドメイン、プロンプトインジェクション、紛らわしい文字、ポリシールール |
| `veritas_os/core/value_core.py` | 9次元加重の価値関数、EMAによるオンライン学習、TrustLogからの自動リバランス |
| `veritas_os/api/governance.py` | ホットリロード、変更コールバック、監査証跡、価値ドリフト監視付きポリシーCRUD |
| `veritas_os/logging/trust_log.py` | ハッシュチェーンTrustLog `h_t = SHA256(h_{t-1} ∥ r_t)` スレッドセーフ追記 |
| `veritas_os/audit/trustlog_signed.py` | Ed25519署名付きTrustLog、WORMミラー対応 |
