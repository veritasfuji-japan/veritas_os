...
- **PostgreSQLドリルRunbook**: [`docs/postgresql-drill-runbook.md`](docs/postgresql-drill-runbook.md)
...
- **バックエンドパリティカバレッジ**: [`docs/BACKEND_PARITY_COVERAGE.md`](docs/BACKEND_PARITY_COVERAGE.md)
- **レガシーパスクリーンアップ**: [`docs/legacy-path-cleanup.md`](docs/legacy-path-cleanup.md)
...
完全な手順（ロールバック含む）は[`docs/postgresql-production-guide.md` §11](docs/postgresql-production-guide.md)を参照してください。
...
[`docs/postgresql-drill-runbook.md`](docs/postgresql-drill-runbook.md) を参照:
...
関連: [`docs/database-migrations.md`](docs/database-migrations.md) | [`docs/BACKEND_PARITY_COVERAGE.md`](docs/BACKEND_PARITY_COVERAGE.md) | [`docs/legacy-path-cleanup.md`](docs/legacy-path-cleanup.md)
...
- ✅ PostgreSQLストレージバックエンド: MemoryOS・TrustLog用プラガブルバックエンド（Alembicマイグレーション、アドバイザリロックチェーン直列化、195+パリティテスト）。JSONL → PostgreSQLインポート手順、スモーク/リリースバリデーション統合、レガシーパスクリーンアップ含む。[`docs/postgresql-production-guide.md`](docs/postgresql-production-guide.md) 参照。
- ✅ PostgreSQL本番ハードニング: 競合テスト（25テスト）、プール/アクティビティメトリクス（28テスト）、バックアップ/リストア/リカバリドリルスクリプトとテスト（31テスト）。[`docs/postgresql-drill-runbook.md`](docs/postgresql-drill-runbook.md) 参照。
...
- ✅ ガバナンス成果物署名: Ed25519署名ポリシーバンドル、ランタイム署名検証、意思決定出力へのガバナンスID。[`docs/governance_artifact_lifecycle.md`](docs/governance_artifact_lifecycle.md) 参照。
- ✅ S3 Object Lock TrustLogミラー: WORM準拠ミラーバックエンド（保持、legal hold、リモート検証）。[`docs/postgresql-drill-runbook.md`](docs/postgresql-drill-runbook.md) 参照。
