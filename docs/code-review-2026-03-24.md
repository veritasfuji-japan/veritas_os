...
| # | 問題 | ステータス | 対応内容 |
|---|------|-----------|---------|
| 1 | `typing.Any` が93箇所 | 未対応 | 段階的に型を厳密化する必要あり（スコープ大） |
| 2 | エラーハンドリングの粒度不足 | 未対応 | 包括的例外キャッチの個別対応が必要（スコープ大） |
| 3 | circular import回避の遅延インポート多用 | 未対応 | アーキテクチャ変更が必要（スコープ大） |
| 4 | Pickle移行ガイドの不足 | 未対応 | ドキュメント整備が必要 |
| 5 | Web検索のprompt injection検出が正規表現のみ | 未対応 | Unicode変種対応は追加検討が必要 |
| 6 | 認証メトリクスがメモリ蓄積のみ | 未対応 | 外部メトリクスストア連携が必要（スコープ大） |
| 7 | SSE認証エラー時のUX | 未対応 | フロントエンド改修が必要 |
| 8 | 型キャスト `as Partial<T>` の多用 | 未対応 | 段階的にバリデーション関数へ移行（スコープ大） |
| 9 | CodeQL: `upload: false` | **対策済み** | `codeql.yml`: GitHub default setup が有効なため意図的に `false` 設定。コメントで理由を明記済み |
| 10 | Dockerfile: 全インターフェース(0.0.0.0)バインド | **対策不要** | コンテナ内では `0.0.0.0` バインドが標準。Docker network isolation により外部アクセスは制御される |
| 11 | setup.sh: `.env`パーミッション自動設定なし | **修正済み** | `setup.sh`: `.env` 作成直後に `chmod 600` を追加。秘密鍵を含むファイルの読取権限を所有者のみに制限 |
| 12 | CORS設定がlocalhostデフォルト | **対策済み** | `route-config.ts`: production環境では `null` を返しフォールバックを無効化。`VERITAS_ENV=production` 時のfail-fastチェックで保護済み |
