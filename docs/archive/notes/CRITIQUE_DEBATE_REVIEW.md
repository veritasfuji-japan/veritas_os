...
| 安全性 | A | ハードブロック+危険テキスト検出+文脈判定+instruction/actionable優先ブロック+難読化語検出に加え、拒否文脈の誤検知抑制・規制グレーゾーン検出・ゼロ幅/Unicode同形異体字の難読化検出を実装。さらに「拒否文脈+実行可能手順の同居」を危険側へ優先する判定順序へ改修し、拒否文面偽装のすり抜け耐性を強化。加えて `違反/禁止` キーワードで `問題なし` など汎用否定語により誤って加点抑制されるケースを排除し、法務/規約リスクの取りこぼしを低減。今回、`対策/予防/security` 等の強 benign 語が共存しても、フィッシング/詐欺系の高リスク領域は危険判定を維持するガードを追加 |
...
26. **[高][完了] D-1 継続: 拒否文脈判定の優先順位を是正し、偽装回避耐性を強化**
   - `_looks_dangerous_text()` の判定順序を変更し、`dangerous/actionable/instructional` の各 intent cue を **拒否文脈より先に評価** するよう修正。
   - これにより、`"I cannot provide ... but ... tutorial/deploy steps"` のような **拒否前置き付きの実行手順提示** を危険判定できるよう改善。
   - 単体テストを追加し、(a) 純粋な拒否文脈は許可維持、(b) 拒否文脈+実行可能記述はブロック、の双方を確認。

27. **[高][完了] D-2 継続: `違反/禁止` の否定判定をキーワード専用化**
   - `_is_keyword_negated()` を拡張し、日本語キーワード `違反` / `禁止` では `問題なし` 等の汎用否定語を使わず、キーワード専用の否定表現（`違反なし`, `禁止されていない` など）のみを適用するよう変更。
   - これにより、`「問題なしだが規約違反」` のような混在文脈で `違反/禁止` シグナルが誤って打ち消される問題を防止。
   - 単体テストを追加し、(a) `問題なし` が含まれても `違反` 加点が維持される、(b) `問題なし` が含まれても `禁止` 加点が維持される、ことを確認。

28. **[高][完了] D-1 継続: 強 benign 語による金融犯罪ドメインの過剰免責を防止**
   - `_NON_BENIGN_OVERRIDE_TERMS` を導入し、`フィッシング/詐欺/闇バイト/不正アクセス` などの高リスク領域は `対策/予防/security` などの強 benign 語が共存しても危険判定を維持する仕様へ変更。
   - 一方で拒否文脈（`cannot provide ...`）は従来どおり優先し、非アクショナブルな拒否応答の誤検知増加を抑制。
   - 単体テストを追加し、(a) 強 benign 語 + フィッシング文脈は危険判定、(b) フィッシング語を含む拒否応答は非危険判定、を確認。
...
- 追加で、金融犯罪・不正アクセス領域は強 benign 語が共存しても危険判定を維持し、**「防御目的」を偽装した詐欺/フィッシング誘導**の見逃しリスクを低減した。
...
- 拒否文脈より先に intent cue（dangerous/actionable/instructional）を評価する順序修正により、拒否文面偽装リスクは低減した。ただし、**より婉曲な手順提示（暗喩・符丁・分割投稿）**は残存しうるため、監査ログベースのルール更新を継続すること。
