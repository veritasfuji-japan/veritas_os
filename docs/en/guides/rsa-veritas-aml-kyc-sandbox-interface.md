...
- VERITAS_RSA_SANDBOX_ALLOW_RAW_UPSTREAM is strip-normalized before equality check to `1`.
...
- Sandbox receiver execution itself is blocked in VERITAS_ENV values prod/production/stg/staging unless VERITAS_RSA_SANDBOX_ENABLE=1.
- This is limited masking for specific fields only, not generalized PII/secret detection.
- Raw output is test/debug-only and must not be used for production audit persistence; route through TrustLog redaction/sanitization pipeline.
