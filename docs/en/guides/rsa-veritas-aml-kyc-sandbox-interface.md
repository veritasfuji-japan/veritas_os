...
- original_llm_intent is `[REDACTED]` by default.
- rsa_action_taken is `[REDACTED]` by default.
- Raw upstream values are returned only when `include_raw_upstream_fields=True` for sandbox debugging.
...
- original_llm_intent and rsa_action_taken are masked as `[REDACTED]` by default.
- Raw values are returned only when `include_raw_upstream_fields=True` for sandbox debugging.
- This is limited default masking for specific fields only.
- This fixture does not perform generalized PII/secret detection or comprehensive redaction across all upstream fields.
