...
| Reviewer-facing performance evidence | CI-safe local harness exports API route, Bind boundary, and TrustLog append latency evidence with p50/p95/p99 and interpretation boundaries. | `docs/en/validation/performance-evidence.latest.json`; `docs/en/validation/performance-evidence.latest.md`; `scripts/performance/export_performance_evidence.py` | Freshness guard (`python -m scripts.performance.check_performance_evidence_freshness`) and artifact schema checks | Not a production SLA; excludes external LLM and customer infrastructure latency unless explicitly measured. | Implemented / Partial |
