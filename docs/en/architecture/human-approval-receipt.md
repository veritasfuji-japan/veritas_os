...
- `VerifiedHumanApprovalReceipt` is the runtime verified proof object. It carries the validated receipt plus verifier-derived signer metadata, signer policy provenance, `signature_verification_reason`, `verified_at`, `verification_source`, and `verification_proof_hash` computed over canonical verifier-derived proof fields including `signer_policy_hash`. Artifact/envelope signer self-claims are never copied in as authorization truth.
