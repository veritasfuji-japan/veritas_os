...
current builder deliberately cannot issue one. The deterministic,
...
`LABA_ARCHITECTURE_GAP_UNVERIFIED_REAL_GOVERNANCE_ARTIFACTS` after source
...
The artifact is single-use, binds a deterministic idempotency identity, requires
...
authorization consumed. The next PR must implement the **Bind Authorization
Consumption / Bind Invocation Gate** and enforce atomic single-use consumption.
