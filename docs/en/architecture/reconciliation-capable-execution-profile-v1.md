...
> reconciliation, the target must present current, independently anchored
> reconciliation capability evidence before the native v2 authorization may be
> consumed.
...
-> current reconciliation-capability evidence recheck
-> fail closed if the required capability is absent or invalid
...
- expected verifier policy hash; and
...
authenticate itself merely by repeating the expected verifier labels.
...
When the gate is required, the evidence must be structurally valid and currently
admissible under the existing reconciliation capability validator.
...
The reconciliation capability artifact remains evidence, not authority.
...
1. current `AUTHORITATIVE_QUERY` passes when required;
2. current `AUTHORITATIVE_EVIDENCE` passes when required;
3. policy-not-required preserves the existing consumption contract;
4. missing evidence fails before consumption;
5. `HEURISTIC_ONLY` fails before consumption;
6. `UNAVAILABLE_OR_UNVERIFIED` fails before consumption;
7. expired evidence fails before consumption;
8. endpoint drift fails before consumption;
9. target configuration drift fails before consumption;
10. verifier trust-anchor mismatch fails before consumption;
