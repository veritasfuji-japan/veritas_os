...
- [x] production で sanitize import failure を fail-closed にする
  ...
  - 2026-03-23 追記。`veritas_os/api/routes_system.py` の `/status` / `/v1/status` / `/api/status` でも `status` / `checks` / `runtime_features` / `auth_store` / `trust_log` / `memory_health` を返すようにし、軽量な status poll のみを利用する運用でも Memory 破損フォールバックや安全機能欠落を silent degradation のまま見逃しにくくした。既存の `pipeline_ok` / `api_key_configured` 契約は維持している。
