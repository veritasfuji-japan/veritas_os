...
| P5 | #2 広域例外握りつぶし（Low-Med） | `pipeline_retrieval.py` は 3箇所すべて `logger.exception()` 対応済み。`pipeline.py` は 9箇所すべて `logger.debug(..., exc_info=True)` 対応済み。 | ✅ 完了 |
...
3. **pipeline.py の広域 except Exception**: 意図的設計で 9箇所存在し、現在は 9/9 が `exc_info=True` 付きログ出力に統一済み。段階的に `logger.debug` → `logger.warning` への昇格、または Structured logging 強化が望ましい。
...
- 追補（2026-03-15 再改善）: `pipeline.py` line 817 の `call_core_decide._params` 例外ログに `exc_info=True` を追加し、レビューで残存扱いだった 1 箇所を解消。対応テスト `test_call_core_decide_logs_exc_info_on_signature_inspection_failure` を追加。
...
2. **`pipeline.py` broad exception ログの改善反映**: line 817 の `_params` 例外ログに `exc_info=True` を追加し、9箇所すべてでスタックトレース付きログを出力（Sec 4, 5, 12, 追加改善 #9）
...
| 追加改善 #9 | line 817 の `signature inspection` ログで `exc_info=True` 欠落 | `exc_info=True` を追加して 9/9 統一 | 解消済み |
