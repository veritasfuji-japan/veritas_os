...
9. **#3 改善（2026-03-15 追補4）**: `pipeline.py` の `call_core_decide` におけるシグネチャ検査フォールバック（意図的 broad exception）へ `logger.debug(..., exc_info=True)` を追加し、例外握りつぶしではなく運用時の原因追跡を可能化。
...
| P5 | #2 広域例外握りつぶし（Low-Med） | `pipeline_retrieval.py` / `pipeline.py` の意図的 broad exception にスタックトレース付きログ（`logger.exception`, `exc_info=True`）を追加し、運用可観測性を改善。 | ✅ 完了 |
