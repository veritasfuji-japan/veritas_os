...
- 残課題: `pipeline.py` オーケストレーション層の広域 `except Exception` は意図的設計（subsystem resilience）で 9箇所存在し、全箇所にコメントで理由が明記されている。8箇所は `logger.debug(..., exc_info=True)` でスタックトレース付きログを出力するが、1箇所（line 817: `_params` の signature inspection）のみ `exc_info=True` が欠落しており、可観測性に軽微なギャップが残る。
- **score: 82/100**（前回 76）
...
- 安全系の `except Exception` 握りつぶし問題は `pipeline_policy.py` で解消（8箇所すべてコンテキスト特化型限定タプル）。`pipeline.py` には 9箇所が意図的に残存し、全箇所にコメント（`# subsystem resilience:`）が明記、8/9箇所が `exc_info=True` 付きログを出力（1箇所 line 817 のみ欠落）。`pipeline_retrieval.py` では 3箇所すべて `logger.exception()` で出力。
...
- `pipeline.py:817` の `exc_info=True` 欠落は Security 観点ではなく Code Quality 観点の問題だが、signature inspection 失敗時の原因追跡性に影響する。
- **score: 80/100**（前回 71、+9pt）。レビュー精査により 81→80 に 1pt 下方修正（毒性フィルタのパターン限界とログ欠落を考慮）。
...
| 2 | 安全系の広域例外握りつぶし | **大幅改善**: pipeline_policy は限定例外化（8箇所全てコンテキスト特化タプル）。pipeline.py は 9箇所意図的残存（8/9 が exc_info=True、1箇所 line 817 欠落）。pipeline_retrieval.py は 3箇所 logger.exception() 対応済み | Low-Med |
...
9. **#3 改善（2026-03-15 追補4）**: `pipeline.py` の `call_core_decide` におけるシグネチャ検査フォールバック（意図的 broad exception）へ `logger.debug(...)` を追加。ただし `_params` 関数（line 817）では `exc_info=True` が欠落しており、`_can_bind` 側（line 828-831）のみ `exc_info=True` 付き。完全な可観測性には line 817 への `exc_info=True` 追加が必要。
...
| P5 | #2 広域例外握りつぶし（Low-Med） | `pipeline_retrieval.py` は 3箇所すべて `logger.exception()` 対応済み。`pipeline.py` は 9箇所中 8箇所が `logger.debug(..., exc_info=True)` 対応済み（line 817 の `_params` のみ `exc_info=True` 欠落）。 | ✅ 概ね完了（1箇所残存） |
...
3. **pipeline.py の広域 except Exception**: 意図的設計で 9箇所存在、8/9 が `exc_info=True` 付きログ出力済み。line 817（`_params` の signature inspection）のみ `exc_info=True` 欠落。段階的に `logger.debug` → `logger.warning` への昇格、または Structured logging 強化が望ましい。
...
| 2 | ✅ 完了 | `pipeline_policy.py` 内 8箇所すべてコンテキスト特化型限定例外タプル（`(R,V,T,A)` ×2、`(K,T,A)` ×1、`(V,T)` ×5）。`pipeline.py` は 9箇所が意図的残存（全箇所コメント明記、8/9 が `exc_info=True`、line 817 のみ欠落） |
...
- 理由: 全 20 項目の実装をコード実読で確認し、3,202 テスト実行で裏取り済み。独立再検証（2026-03-15）では主要主張 25項目中 23項目が正確、2項目が軽微な記述不正確（例外タプルの均一性表記、`exc_info` 欠落 1箇所）であり、実質的な虚偽記載なし。テスト関数定義数 3,286（grep ベース）と報告値 3,227（passed+failed+skipped）の差分は pytest パラメタライズ・動的スキップで説明可能。外部インフラ実環境検証は引き続き範囲外。
...
2. **`pipeline.py` broad exception ログの精密化**: 9箇所中 8箇所が `exc_info=True` 対応、line 817 のみ欠落を明記（Sec 4, 5, 12, 追加改善 #9）
3. **Security スコア下方修正**: 81→80（-1pt）。毒性フィルタの基本パターン限界とログ欠落を反映（Sec 6, 11）
...
| 追加改善 #9 | `logger.debug(..., exc_info=True)` を追加と記載 | line 817 のみ `exc_info=True` 欠落を明記 | 低（9箇所中 1箇所の欠落） |
...
3. **`pipeline.py` の `logger.debug` レベル**: subsystem resilience 例外は全て `DEBUG` レベルで出力。本番運用では DEBUG ログが無効化される可能性があり、`WARNING` レベルへの昇格を検討すべき。
