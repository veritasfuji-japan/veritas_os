...
### 今回完了した改善
- **Priority 1 / 指示 1-2 追加（今回）**: `veritas_os/core/fuji.py` に残っていた policy hot reload の compatibility-heavy な inline 実装を、共有 helper である `veritas_os/core/fuji_policy.py` の `_check_policy_hot_reload()` へ委譲する形に整理した。`fuji.py` 側は capability alias 同期と shared policy state の反映だけを担い、policy file の監視・読み込み・runtime pattern 再構築は shared helper 側へ一元化した。既存の `fuji.POLICY` / `_POLICY_MTIME` 互換 contract は維持し、`veritas_os/tests/test_fuji_core.py` に委譲経路の回帰テストを追加した。
