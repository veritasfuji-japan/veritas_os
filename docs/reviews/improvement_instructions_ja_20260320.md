...
- **Priority 3 / 指示 3-2 追加**: `pipeline_helpers.py` の broad `except Exception` 7箇所を限定例外タプルへ縮小した（`_as_str` → `(TypeError, ValueError, AttributeError)`、`_norm_severity` → `(TypeError, ValueError, AttributeError)`、`_to_bool_local` → `(TypeError, ValueError, AttributeError)`、`_set_int_metric` → `(TypeError, ValueError)`、`_set_bool_metric` → `(TypeError, ValueError, AttributeError)`、`_query_is_step1_hint` → `(TypeError, AttributeError)`、`_has_step1_minimum_evidence` → `(TypeError, AttributeError, KeyError)`）。`_lazy_import` は外部モジュール読込のため意図的に broad を維持した。回帰テストを `test_pipeline_signature_adapter.py` に追加した。
- **Priority 3 / 指示 3-2 追加**: `pipeline_contracts.py` の `_ensure_full_contract` 内 broad `except Exception` 4箇所を限定例外タプルへ縮小した（stage_latency → `(TypeError, ValueError)`、mem_evidence_count → `(TypeError, ValueError)`、context_obj → `(TypeError, ValueError)`、memory_meta.query → `(TypeError, AttributeError)`）。`_deep_merge_dict` と `_merge_extras_preserving_contract` の外側 recovery handler は最終防壁のため意図的に broad を維持した。回帰テストを `test_pipeline_signature_adapter.py` に追加した。
