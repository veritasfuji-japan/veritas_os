...
Intervention Actionability Map v0 は、Governance Safeguard Coverage Matrix v0
の次に置く compact な guidance layer です。これまでの layer は、trajectory
の劣化、irreversibility marker、Actor Recognition Gap、governance attack
surface、safeguard evidence を可視化してきました。この map は、その visibility
と automatic action を分離したまま、「governance marker が見えたとき、代表的に
どの intervention category が actionability を持つか」を問います。

Demo payload では additive field として
`governance_layer_snapshot.intervention_actionability_map` に公開されます。
`first_structural_degradation_signal`、`early_warning`、
`last_meaningful_intervention`、`irreversibility_horizon`、
`actor_recognition_gap`、`bind_after_recognition_gap`、`self_authorization`、
`evidence_chain_manipulation`、`approval_receipt_spoofing`、
`escalation_suppression` などの visible marker を、`annotate`、`warn`、
`preserve_evidence`、`reframe`、`pause`、`escalate`、
`require_explicit_approval`、`freeze_bind_path`、`post_horizon_review` などの
representative category に接続します。各 mapping は、後続レビューのために preserve
すべき evidence も明示します。

この layer は methodological restraint を保ちます。automatic enforcement、automatic
blocking、automatic escalation、scoring、certification、production decisioning、formal
...
Governance Evidence Packet v0 は、Intervention Actionability Map v0 の次に置く
compact な reviewer packet です。これまでの layer は marker、registry、matrix、
representative actionability guidance を出力してきました。この packet は、それらの既存
layer を reviewer が点検しやすい 1 つの summary に束ねます。

Demo payload では additive field として
`governance_layer_snapshot.governance_evidence_packet` に公開されます。
`pre_boundary_collapse` demo のための packet であり、runtime bind behavior、OpenAPI、
state family、pre-bind vocabulary、Trajectory Shaping Lineage v0、Governance Attack
Surface Registry v0、Governance Safeguard Coverage Matrix v0、Intervention Actionability
Map v0 は変更しません。

v0 packet は次を要約します。

- decision context と bind outcome
- trajectory shaping lineage
- dynamic degradation signals
- irreversibility horizon markers
- actor recognition gap markers
- governance attack surface registry refs
- safeguard coverage matrix refs
- intervention actionability map refs
- reviewer が点検すべき preserved evidence refs
- reviewer questions と explicit limitations

これは deterministic representative reviewer packet です。decision-space narrowing、
governability degradation、safeguard visibility、representative intervention categories が
既存 evidence layer の間でどのようにつながるかを読みやすくし、reviewer readability を改善します。

この packet は methodological restraint を維持します。certification、automatic enforcement、
