...
| **第13条**（透明性） | ⚠️ 部分準拠 | ⚠️ 部分準拠 | ユーザーガイド・第三者通知対応・**パイプラインでの`ai_disclosure`/`regulation_notice`自動注入** |
...
- `classify_annex_iii_risk()` にスペース挿入回避検出を追加し、`_normalise_text()` との正規化一貫性を確保（✅ GAP-01d強化）
...
- `docs/archive/notes/`配下の各種レポート（TRUSTLOG_VERIFICATION, CODE_REVIEW_PRINCIPLESなど）
...
2. ~~**変更管理プロセスの正式化**~~: ✅ **対応済** — `validate_change_management()` 関数を追加し、`docs/eu_ai_act/change_log.json` による変更記録の構造化検証を実装。`validate_deployment_readiness()` に統合し、デプロイ前に変更管理記録の存在・整合性を自動チェック。
...
- `eu_compliance_pipeline` デコレータで `ai_disclosure` / `regulation_notice` を全レスポンスに自動注入（✅ Art.13/50パイプライン統合）
- 高リスク決定時に `affected_parties_notice` を `ThirdPartyNotificationService` 経由で自動生成・注入（✅ GAP-17パイプライン統合）

**残存ギャップ:**

1. ~~**エンドユーザー向け透明性の運用化**~~: ✅ **大幅改善** — `eu_compliance_pipeline` デコレータが `ai_disclosure` / `regulation_notice` を全レスポンスに自動注入するようになり、API消費者側での実装漏れリスクを軽減。フロントエンド `EUAIActDisclosure` コンポーネントも実装済み（GAP-13b）。
...
- `eu_compliance_pipeline` デコレータが `ai_disclosure` / `regulation_notice` を全レスポンスに自動注入（✅ Art.50パイプライン統合）

**残存ギャップ:**

1. ~~**UI表示の未確認**~~: ✅ **対応済** — `eu_compliance_pipeline` で `ai_disclosure` / `regulation_notice` が全レスポンスに自動注入されるようになり、フロントエンド `EUAIActDisclosure` コンポーネント（GAP-13b）と合わせてUI表示を確保。
...
| GAP-01d | Art.9 | リスク分類の回避耐性なし | `classify_annex_iii_risk()` にNFKC/ホモグリフ/ハイフン正規化・**スペース挿入回避検出**を適用 |
...
| Art.9 | `core/eu_ai_act_compliance_module.py` | 340-368 | `classify_annex_iii_risk()` NFKC/ホモグリフ/スペース挿入正規化 (GAP-01d強化) |
