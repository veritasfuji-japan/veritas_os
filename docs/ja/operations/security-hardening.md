...
## Type B Summary: Performance evidence artifact（英語正本準拠）
...
- この証跡は CI-safe local の reviewer 向け性能証跡であり、本番 SLA 主張ではありません。
- 外部 LLM provider 遅延・顧客インフラ遅延は（明示測定されない限り）含みません。顧客PoC環境で再測定が必要です。
