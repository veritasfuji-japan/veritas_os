...
- Anthropic については、request formatting、response parsing、header construction、model allowlist、planned-tier warning、sanitized error handling の offline contract test を追加しています。ただし tier は Planned のままであり、live integration validation、運用レビュー、benchmark evidence、利用組織側のprovider policy review なしに production-supported と扱ってはいけません。
