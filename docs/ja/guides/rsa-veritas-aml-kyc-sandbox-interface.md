...
- include_raw_upstream_fields=True には、次の両方が truthy value である必要があります。
  - VERITAS_RSA_SANDBOX_ENABLE
  - VERITAS_RSA_SANDBOX_ALLOW_RAW_UPSTREAM
- VERITAS_ENV が prod / production / stg / staging の場合、raw 出力は許可されません。
- VERITAS_ENV 判定前に strip/lower 正規化を行います。
- truthy 判定は strip/lower 正規化後に 1 / true / yes / on を受け付けます。
