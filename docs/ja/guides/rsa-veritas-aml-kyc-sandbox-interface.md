...
- VERITAS_RSA_SANDBOX_ALLOW_RAW_UPSTREAM は strip 正規化後に `1` と比較されます。
- VERITAS_ENV が prod / production / stg / staging の場合、raw 出力は許可されません。
- VERITAS_ENV 判定前に strip/lower 正規化を行います。
- sandbox receiver 自体は VERITAS_ENV が prod / production / stg / staging の場合に
  VERITAS_RSA_SANDBOX_ENABLE=1 がない限り実行拒否されます。
- これは特定フィールドに限定したマスキングであり、汎用的な PII/secret 検知ではありません。
- raw 出力はテスト/デバッグ専用であり、本番監査永続化には使用せず、
  TrustLog redaction/sanitization pipeline を必ず通してください。
