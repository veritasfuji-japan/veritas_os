# ドキュメント（日本語）

このディレクトリは日本語ドキュメントの入口です。

総合バイリンガルインデックス: [../INDEX.md](../INDEX.md)
英日対応状況: [../DOCUMENTATION_MAP.md](../DOCUMENTATION_MAP.md)

English version: [../en/README.md](../en/README.md)

---

## セクション

### 運用・ランブック
- [Enterprise SLO/SLI 運用Runbook](operations/enterprise_slo_sli_runbook_ja.md)
- [ランタイムデータポリシー](operations/runtime-data-policy.md)

### 検証・テスト
- [テストカバレッジレポート](validation/coverage-report.md)
- [テスト強化後カバレッジ追跡](validation/coverage-after-hardening.md)

### ガバナンス・コンプライアンス
- [EU AI Act 準拠レビュー](governance/eu-ai-act-compliance-review.md)
- [エンドユーザー向け EU AI Act 使用説明書](governance/user-guide-eu-ai-act.md)
- [Policy-as-Code 解説](governance/policy-as-code.md)

### ガイド
- [VERITAS OS 総合ユーザーマニュアル](guides/user-manual.md)
- [FUJI エラーコードリファレンス](guides/fuji-error-codes.md)
- [FUJI EU AI Act & Enterprise Strict Pack 使い方](guides/fuji-eu-enterprise-strict-usage.md)
- [自己修復ループ解説](guides/self-healing-loop.md)
- [金融 PoC パック（1日クイックスタート）](guides/poc-pack-financial-quickstart.md)

### コードレビュー
- [レビュー文書インデックス](reviews/README.md)
- [レビュー文書マップ](reviews/code-review-document-map.md)
- [レビューステータス](reviews/code_review_status_ja.md)
- [ここから始める](reviews/codex_start_here_ja.md)

### 監査
- [Markdown 情報設計監査](audits/markdown_information_architecture_audit_2026_04_07_ja.md)
- [リプレイ監査](audits/replay_audit_ja.md)

### アーキテクチャ（共有）
- [アーキテクチャ文書](../architecture/) — EN（Continuation Runtime, 責務境界, TrustLog）

### EU AI Act（共有）
- [EU AI Act コンプライアンス成果物](../eu_ai_act/) — EN（リスク評価, モデルカード, バイアス）

### UI / フロントエンド（共有）
- [UI ドキュメント](../ui/) — Mixed（アーキテクチャ, 統合計画, プレビュー）
