...
- staged readiness report v2.1 では、`deployment_ready` は
  ...
  `compose_validation` が未添付、または読み込み失敗で `null` になった場合、
  `overall_readiness.compose_validated=true` は
  「compose の失敗が添付されていない」ことを示すだけで、
  compose 検証が実行・読込済みである証明ではありません。
...
- `live_provider_validation` が未添付、または読み込み失敗で `null` になった場合、
  `overall_readiness.live_provider_ok=true` は
  live provider 検証が実行・読込済みであることを意味しません。
  生成時の警告ログと、compose / live provider レポートファイルの存在を
  確認してください。
