...
作成日: 2026-03-19
対象文書: `docs/ja/reviews/precision_code_review_ja_20260319.md`
...
再評価文書を全文確認したうえで boundary checker の運用安定性を再点検したところ、
`extract_doc_extension_points()` は `-` / `*` の箇条書きしか読めず、
**同じ意味の Markdown でも `+` 箇条書きや番号付きリストへ整形しただけで
`doc_alignment_error` が発生しうる** 状態でした。

これは Planner / Kernel / FUJI / MemoryOS の責務や public contract の問題ではなく、
文書整合性チェックが Markdown 方言差分に過敏なことによる運用ノイズです。
そのため今回は、中核モジュールには触れず、
**boundary checker の文書パーサだけを最小改善**しました。

- `scripts/architecture/check_responsibility_boundaries.py` の
  `extract_doc_extension_points()` を修正し、`+` 箇条書きと `1.` 形式の番号付きリストも
  Preferred extension points として抽出できるように改善
- `veritas_os/tests/test_responsibility_boundary_checker.py` に回帰テストを追加し、
  Markdown の一般的な箇条書き記法差分だけでは CI が誤検知しないことを固定

この対応も Planner / Kernel / FUJI / MemoryOS の責務境界や既存 API 契約を変えず、
レビューで継続課題とされた「正規拡張ポイントの明確化」を支える
文書整合性チェックの信頼性だけを上げるものです。

## 2026-03-20 追加改善（CRLF 改行の architecture 文書でも drift 誤検知しないよう修正）

再評価文書を全文確認したうえで boundary checker の文書抽出条件を再点検したところ、
節抽出の正規表現が `\n` 前提になっており、**`docs/architecture/core_responsibility_boundaries.md` を
Windows 系ツール経由で CRLF 改行に整形しただけでも section 抽出に失敗し、
`doc_alignment_error` が誤検知されうる** 状態でした。

これは Planner / Kernel / FUJI / MemoryOS の責務や public contract の問題ではなく、
文書整合性チェックが改行コード差分に過敏なことによる運用ノイズです。
無駄な中核変更を避けるため、今回は boundary checker の文書パーサだけを最小差分で改善しました。

- `scripts/architecture/check_responsibility_boundaries.py` の section 抽出を修正し、
  LF だけでなく CRLF 改行でも Preferred extension points 節を抽出できるよう改善
- `veritas_os/tests/test_responsibility_boundary_checker.py` に CRLF 文書の回帰テストを追加し、
  改行コード正規化だけで CI が誤検知しないことを固定
