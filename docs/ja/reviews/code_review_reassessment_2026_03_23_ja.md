...
`docs/ja/reviews/code_review_2026_03_21_ja.md` を全文読了した上で、現行コードベースと主要ドキュメントを再確認し、**2026-03-23 時点の再評価**を行った。
...
- **P1 対応 / degraded の運用意味固定**: `docs/ja/operations/enterprise_slo_sli_runbook_ja.md` に `sanitize` / `atomic_io` / `auth_store` / `memory` / `trust_log` の degraded を優先度つきで扱う alert policy と、`/health` / `/status` の `status=degraded` をアラート対象にする運用判定を追記した。これにより、可視化済みの degraded state を運用アクションへ結びつける最低限の契約を文書として固定した。
...
- **P1 対応 / degraded alert semantics の API 契約化**: `veritas_os/api/routes_system.py` に `alert_policy` を追加し、`/health` / `/status` が `checks.*`・`runtime_features.*` から `highest_priority`・`requires_action`・`alerts[]` を機械可読で返すようにした。runbook 上の P0/P1 判断を監視側が独自実装なしで取り込めるようになり、`status=degraded` を見えているだけで放置する運用リスクをさらに下げた。回帰防止のため `veritas_os/tests/test_api_backend_improvements.py` に health/status の alert policy テストを追加した。
