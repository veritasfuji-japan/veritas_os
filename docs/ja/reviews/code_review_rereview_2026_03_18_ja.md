...
- 対象:
  - `docs/ja/reviews/code_review_full_2026_03_16_agent_ja.md` の全文
...
- `docs/ja/operations/enterprise_slo_sli_runbook_ja.md` に本番デプロイ前の P0 セキュリティ確認項目を追加し、`VERITAS_ENV=production` 必須・`NEXT_PUBLIC_VERITAS_API_BASE_URL` 禁止・`VERITAS_AUTH_ALLOW_FAIL_OPEN` 禁止を runbook に明記した。
...
- さらに、shared `MemoryStore` の helper 参照を `memory.py` 側の `filter_recent_records` / `build_kvs_search_hits` / lifecycle helper 群へ再配線し、既存の monkeypatch ベースのユニットテストや局所差し替えワークフローとの互換性を維持した。
- `veritas_os/tests/test_memory_core.py` に回帰テストを追加し、`memory.filter_recent_records` を patch した場合でも `MemoryStore.recent()` がその差し替えを経由することを固定した。
