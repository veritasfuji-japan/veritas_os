...
### 7.7 2026-03-31 追加改善（最小・HTTP メソッド大文字小文字ゆらぎ対応）

無駄な機能追加は行わず、指摘C（静的突合チェックの検証精度）に限定して最小改善を実施。

- `scripts/quality/check_frontend_api_contract_consistency.py`
  - `method: "post"` のような **小文字メソッド指定** を静的抽出時に正しく解釈できるよう、`method` 抽出正規表現を case-insensitive 化。
  - 抽出結果は従来どおり `POST` などの大文字へ正規化し、BFF/OpenAPI 突合時の誤判定（`GET` 既定値へのフォールバック）を抑制。
- `veritas_os/tests/test_frontend_api_contract_consistency.py`
  - `veritasFetch(endpoint, { method: "post" })` 相当のケースで `POST` が抽出される回帰テストを追加。
...
### 7.9 2026-03-31 追加改善（最小・`"method"` キー表記ゆらぎ対応）

無駄な機能追加は行わず、指摘C（静的突合チェックの検証精度）に限定して最小改善を実施。

- `scripts/quality/check_frontend_api_contract_consistency.py`
  - `const options = { "method": "POST" }` のような **クォート付きキー** を静的抽出できるよう、`method` 抽出正規表現を最小修正。
  - これにより、プロジェクト内のコードフォーマッタ設定差による表記ゆらぎで `GET` へ誤フォールバックするケースを抑制。
- `veritas_os/tests/test_frontend_api_contract_consistency.py`
  - `fetch(endpoint, postOptions)` で `postOptions = { "method": "POST" }` を使った回帰テストを追加。
...
### 7.11 2026-03-31 追加改善（最小・ネスト済み options オブジェクト対応）

無駄な機能追加は行わず、指摘C（静的突合チェックの検証精度）に限定して最小改善を実施。

- `scripts/quality/check_frontend_api_contract_consistency.py`
  - `const requestOptions = { headers: {...}, method: "PATCH" }` のように **ネストしたオブジェクトが `method` より先に現れるケース** で、従来の正規表現が最初の `}` で早期終了し `method` を見落とす問題を修正。
  - options オブジェクト抽出を最小限の静的スキャン（対応する `}` までのバランス走査）へ置換し、`GET` への誤フォールバックを抑制。
- `veritas_os/tests/test_frontend_api_contract_consistency.py`
  - `headers` のネストを含む options オブジェクトから `PATCH` が正しく抽出される回帰テストを追加。

セキュリティ補足:
- 本改善は契約整合性チェックの抽出精度向上のみを対象とし、BFF 許可行列・認可判定・公開 API 挙動は不変。
- 新規の API 公開面は増加していない。なお、動的に生成される options については引き続き静的解析の限界があるため、CI と実行時監査の併用を継続すること。

### 7.12 2026-03-31 追加改善（最小・バッククォート method リテラル対応）

無駄な機能追加は行わず、指摘C（静的突合チェックの検証精度）に限定して最小改善を実施。

- `scripts/quality/check_frontend_api_contract_consistency.py`
  - `const postOptions = { method: \`POST\` }` のような **バッククォート文字列の method リテラル** を静的抽出できるよう、`method` 抽出正規表現を最小修正。
  - これにより、プロジェクト内の文字列表記ゆらぎで `GET` へ誤フォールバックするケースを抑制。
- `veritas_os/tests/test_frontend_api_contract_consistency.py`
  - ``method: `POST` `` を使った options 参照の回帰テストを追加し、再発を防止。

セキュリティ補足:
- 本改善は契約整合性チェックの抽出精度向上のみを対象とし、BFF 許可行列・認可判定・公開 API 挙動は不変。
- 新規の API 公開面は増加していない。静的解析の限界（動的に組み立てた method 値）は引き続き残るため、CI と実行時監査の併用を継続すること。
