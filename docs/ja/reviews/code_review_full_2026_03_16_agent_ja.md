...
- セキュリティ警告:
  - runtime での pickle/joblib 読み込み禁止は維持されるが、検知はファイル走査ベースのため、運用では `docs/en/operations/memory_pickle_migration.md` に従い legacy artifact 自体を配置しないこと。
...
### 28) P0修正: `api/server.py` の Trust Log ロック fallback で lint エラーを解消（完了）
- 背景:
  - `api/server.py` の `_trust_log_lock` fallback で `threading.Lock()` を使用している一方、`threading` import が欠落しており、CI の `ruff check` で `F821 Undefined name 'threading'` が発生していた。
- 変更内容:
  - `veritas_os/api/server.py` の import 群へ `import threading` を復元。
- テスト:
  - `ruff check veritas_os/api/server.py` で lint エラー解消を確認。
  - 既存回帰として `veritas_os/tests/test_api_dependency_resolver.py` と `veritas_os/tests/test_server_coverage.py` を再実行し、互換性を確認。
- セキュリティ警告:
  - Trust log 書き込みは排他制御に依存するため、lock 初期化の失敗や import 欠落は監査ログ整合性の低下につながる。CI lint を必須ゲートとして維持し、監査系コードの未定義名を早期に遮断すること。

### 29) P0追加: 非本番でも Auth failure mode `open` を明示許可制に変更（完了）
- 背景:
  - 既存実装では non-production で `VERITAS_AUTH_STORE_FAILURE_MODE=open` を設定すると、そのまま fail-open が有効化されていた。
  - 検証用途としては有用だが、環境変数の持ち越しや誤設定でセキュリティ前提が崩れる運用事故リスクが残っていた。
- 変更内容:
  - `veritas_os/api/auth.py` の `_auth_store_failure_mode()` を強化し、`open` 指定時は `VERITAS_AUTH_ALLOW_FAIL_OPEN=true` が同時に設定されている場合のみ `open` を許可するよう変更。
  - `VERITAS_AUTH_ALLOW_FAIL_OPEN` が未設定のまま `open` 指定された場合は `closed` にフォールバックし、`[security-warning]` をログ出力する fail-closed 挙動を追加。
  - `veritas_os/tests/test_auth_core.py` の既存 `open` 系テストを明示許可付きケースへ更新し、あわせて「許可フラグなしでは `closed` へ強制される」回帰テストを追加。
- 目的:
  - fail-open を「意図した検証時のみ有効化できる危険オプション」へ格下げし、設定ミス時の防御既定を強化する。
- セキュリティ警告:
  - `VERITAS_AUTH_ALLOW_FAIL_OPEN=true` はテスト限定フラグとして扱い、本番・準本番へ持ち込まないこと。誤って有効化すると auth store 障害時に nonce/replay/rate-limit 防御がバイパスされるリスクがある。

### 30) P0修正: fail-open 明示許可制に伴う回帰テストを CI 期待値へ更新（完了）
- 背景:
  - 29) で `VERITAS_AUTH_STORE_FAILURE_MODE=open` に `VERITAS_AUTH_ALLOW_FAIL_OPEN=true` の同時指定を必須化したため、既存の fail-open 想定テスト 2 件が旧前提のまま失敗していた。
- 変更内容:
  - `veritas_os/tests/test_api_server_extra.py` の fail-open ケースに `VERITAS_AUTH_ALLOW_FAIL_OPEN=true` を追加。
  - `veritas_os/tests/test_auth_store_consistency_chaos.py` の fail-open ケースに `VERITAS_AUTH_ALLOW_FAIL_OPEN=true` を追加。
- テスト:
  - `pytest -q veritas_os/tests/test_api_server_extra.py veritas_os/tests/test_auth_store_consistency_chaos.py veritas_os/tests/test_auth_core.py`
  - `ruff check veritas_os/tests/test_api_server_extra.py veritas_os/tests/test_auth_store_consistency_chaos.py veritas_os/tests/test_auth_core.py`
- セキュリティ警告:
  - fail-open テストを通すための `VERITAS_AUTH_ALLOW_FAIL_OPEN=true` は検証専用。運用環境へ持ち込むと auth store 障害時の防御低下リスクがある。


### 31) P0完了: BFF API Base URL の運用ガイドを server-only 構成へ統一（完了）
- 背景:
  - P0 方針として `NEXT_PUBLIC_VERITAS_API_BASE_URL` 依存の排除は実装済みだったが、運用ドキュメントと compose サンプルには旧来の公開環境変数（`NEXT_PUBLIC_API_BASE_URL`）が残っていた。
  - この不整合により、デプロイ手順をそのまま適用した運用者が公開環境変数へ内部 API 接続先を書き込み、BFF fail-closed や構成露出を招くリスクがあった。
- 変更内容:
  - `.env.example` を `VERITAS_API_BASE_URL`（server-only）へ更新し、`NEXT_PUBLIC_*` 禁止コメントを追記。
  - `docker-compose.yml` の frontend 環境変数を `VERITAS_API_BASE_URL` に変更。
  - `readme_ja.md` と `docs/ui/README_UI.md` のセットアップ手順を server-only 変数前提へ更新し、production での `NEXT_PUBLIC_*` 禁止と fail-closed 挙動を明記。
- 目的:
  - 実装済みのセキュリティ方針と運用手順を一致させ、設定例のコピペによる公開環境変数への内部経路露出を防止する。
- テスト:
  - `docker compose config` で compose 設定の整合性を確認。
- セキュリティ警告:
  - `VERITAS_API_BASE_URL` は server-only 前提であり、公開クライアントへ配布される `NEXT_PUBLIC_*` へ内部 API の実接続先を設定してはならない。production では BFF が 503 fail-closed となる。

### 32) P1継続: `api/server.py` から Trust Log ランタイム責務を分離（完了）
- 背景:
  - `api/server.py` に Trust Log の集約 JSON 読み込み、JSONL 追記、権限 hardening、shadow decide 保存が残っており、ルーティング責務と監査永続化責務が混在していた。
  - 既存の `trust_log_io.py` は低レベル I/O を提供していたが、server 側の runtime 配線と後方互換ラッパは依然 `server.py` に残っていた。
- 変更内容:
  - `veritas_os/api/trust_log_runtime.py` を新設し、`TrustLogRuntime` を実装。
    - `load_logs_json`
    - `secure_chmod`
    - `save_json`
    - `append_trust_log`
    - `write_shadow_decide`
  - `veritas_os/api/server.py` の `_load_logs_json` / `_secure_chmod` / `_save_json` / `append_trust_log` / `write_shadow_decide` は、後方互換を維持した薄い委譲ラッパへ変更。
  - trust log lock の import fallback も runtime 初期化側へ寄せ、`server.py` から直接の排他制御詳細を減らした。
- 目的:
  - `server.py` の責務をさらに薄くし、監査ログ永続化ロジックの変更影響範囲を局所化する。
- テスト:
  - `veritas_os/tests/test_api_trust_log_runtime.py` を追加し、effective path 経由の読み込み、JSONL + 集約 JSON 追記、shadow snapshot 保存を単体検証。
  - 既存の `veritas_os/tests/test_api_server_extra.py` を再実行し、server 互換ラッパが維持されることを回帰確認。
- セキュリティ警告:
  - Trust Log は監査証跡そのものであり、書き込み権限や出力先の誤設定は証跡欠落・漏えいの双方のリスクを持つ。runtime 分離後も `0o600` 権限付与と server-side 出力先固定を維持し、運用ではログ保存先への不要な write 権限を与えないこと。

### 33) P1修正: Trust Log runtime 分離後の server monkeypatch 互換を回復（完了）
- 背景:
  - 32) の runtime 分離後、`server.py` の互換ラッパが初期化時に保持した `effective_log_paths` / `effective_shadow_dir` を使い続けていた。
  - そのためテストや一部運用コードで `server._effective_log_paths` / `server._effective_shadow_dir` を monkeypatch しても反映されず、Trust Log / shadow snapshot の出力先が更新されない回帰が発生した。
- 変更内容:
  - `veritas_os/api/server.py` の `_load_logs_json` / `_secure_chmod` / `_save_json` / `append_trust_log` / `write_shadow_decide` で、呼び出し時に `TrustLogRuntime` へ最新の `effective_log_paths` / `effective_shadow_dir` を再注入するよう修正。
  - `server.threading` を参照する既存テスト互換のため、`threading` import を復元。
- テスト:
  - `veritas_os/tests/test_api_trust_log_runtime.py` に server ラッパ経由でも monkeypatch されたパスへ出力される回帰テストを追加。
  - `veritas_os/tests/test_coverage_boost.py` の該当失敗ケースを含む部分実行、および `pytest -q veritas_os/tests -m 'not slow' --tb=short` を再実行して全体回帰を確認。
- セキュリティ警告:
  - monkeypatch 互換は主にテスト容易性のために維持している。運用環境で出力先を動的変更できる余地を広げすぎると、監査ログの誤配置や権限制御逸脱のリスクが高まるため、本番ではログ出力先設定を固定すること。
