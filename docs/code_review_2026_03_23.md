...
| # | 項目 | 理由 |
|---|------|------|
| 2.3 | X-Forwarded-For スプーフィング | インフラ構成（リバースプロキシ）に依存するため、運用環境の確認が必要 |
| 2.4 | Bandit スキャン範囲 | CI パイプライン変更は個別 PR で対応推奨 |
| 2.7 | replay_engine monkey-patch | 依存性注入への変更は広範なリファクタリングが必要 |
| 2.8 | useDecide メッセージ ID 衝突 | フロントエンドの状態管理変更は個別対応推奨 |
| 2.9 | useAuditData リクエスト重複排除 | フロントエンドの UX 改善は個別対応推奨 |
| 2.10 | useGovernanceState window.confirm | フロントエンドの UI 変更は個別対応推奨 |

### Low (10件 — 未対応)

Low 項目は機能的影響が小さく、個別に対応可能なため今回は見送り。1.4 対応時に 3.9 (provenance/SBOM) は併せて有効化済み。
