...
| 安全性 | A- | ハードブロック+危険テキスト検出+文脈判定+instruction/actionable優先ブロック+難読化語検出を実装 |
| テスト容易性 | B+ | JSON救出ヘルパー分離と安全ヒューリスティクス単体テスト拡充で改善 |
| 型の一貫性 | A- | `types.py` と実装の主要乖離（severity/role）を解消 |
| 保守性 | B+ | severity共通化・定数化で改善。ただし辞書駆動ヒューリスティクスの運用負荷は残る |

---

## 6. 推奨アクション（優先度順）

1. **[高] D-1 継続**: 危険語辞書の定期更新（スラング・多言語・難読化パターン）と監査ログに基づくルール改善
...
- 否定語判定はキーワード専用化で `illegal/ban` の誤打ち消しを軽減したが、**法令順守を示す婉曲表現（"regulatory exception", "grey zone" など）**は未網羅であり、法務ポリシー連携による補完が必要。
