...
| 安全性 | A | ハードブロック+危険テキスト検出+文脈判定+instruction/actionable優先ブロック+難読化語検出に加え、拒否文脈の誤検知抑制・規制グレーゾーン検出・ゼロ幅/Unicode同形異体字の難読化検出を実装。さらに「拒否文脈+実行可能手順の同居」を危険側へ優先する判定順序へ改修し、拒否文面偽装のすり抜け耐性を強化 |
