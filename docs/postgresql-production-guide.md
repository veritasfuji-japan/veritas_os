...
| Backup/restore/drill scripts | `test_drill_postgres_recovery.py` (31 tests) | Script syntax + coherence |
...
| **Concurrent advisory lock testing** | Advisory lock serialization tested via lock-based contention mock (25 tests in `test_pg_trustlog_contention.py`); real PG contention tested in CI `test-postgresql` job | Lock semantics faithfully emulated; real PG may exhibit different timing under CPU/IO load |
...
|-----------|-------------|------:|
| Hash chain intact under N-way concurrent append | `test_pg_trustlog_contention.py` | 25 |
...
`test_pg_trustlog_contention.py`（25 テスト）が advisory-lock 直列化を検証:
