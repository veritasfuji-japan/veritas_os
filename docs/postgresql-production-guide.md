...
| Advisory lock contention | `test_pg_trustlog_contention.py` (23 tests) | Mock (lock-based) |
| Pool/activity metrics | `test_pg_metrics.py` (28 tests) | Mock pool + TestClient |
| Backup/restore/drill scripts | `test_drill_postgres_recovery.py` (21 tests) | Script syntax + coherence |
...
This test module exercises TrustLog advisory-lock serialization under
realistic concurrent-access patterns using an enhanced mock pool that
faithfully simulates `pg_advisory_xact_lock` via `threading.Lock`:
...
These tests run as part of Tier 1 CI (every PR) and use no live database.
...
| **Concurrent advisory lock testing** | Advisory lock serialization tested via lock-based contention mock (23 tests in `test_pg_trustlog_contention.py`); real PG contention tested in CI `test-postgresql` job | Lock semantics faithfully emulated; real PG may exhibit different timing under CPU/IO load |
...
|-----------|-------------|------:|
| Hash chain intact under N-way concurrent append | `test_pg_trustlog_contention.py` | 23 |
...
| Fail-closed on pool starvation / statement timeout | `test_pg_trustlog_contention.py` | 5 |
| Advisory lock released on both commit and rollback | `test_pg_trustlog_contention.py` | 3 |
...
| Area | Reason |
|------|--------|
| Real PostgreSQL advisory lock timing under CPU/IO saturation | Mock pool uses `threading.Lock`, which has zero-delay acquire |
...
| **Real PG contention CI** | Dedicated CI job running `test_pg_trustlog_contention.py` against a live PostgreSQL service container | Low |
...
`test_pg_trustlog_contention.py`（23 テスト）が advisory-lock 直列化を検証:

- 2 ワーカー同時追記 → チェーン無傷
- N ワーカーバースト（5/10/20）→ ハッシュチェーン有効
- ステートメントタイムアウト → fail-closed
- 接続プール枯渇 → fail-closed
- ロールバック回復 → チェーン状態不変
- スレッドベース競合 → OS スレッドでもチェーン整合性維持
