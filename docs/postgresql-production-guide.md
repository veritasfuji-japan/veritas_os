...
| Advisory lock contention | `test_pg_trustlog_contention.py` (25 tests) | Mock pool (lock-based) |
...
| **Concurrent advisory lock testing** | Mock contention (25 tests); real PG in CI `test-postgresql` job | Mock uses zero-delay lock; real PG timing not tested under load |
