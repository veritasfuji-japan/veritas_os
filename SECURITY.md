...
- **Dependency audit (Node, blocking):** `pnpm audit --prod --audit-level=high`
- **Dependency audit (Node, report):** `pnpm audit --audit-level=high` (non-blocking warning for dev/tooling chains)
