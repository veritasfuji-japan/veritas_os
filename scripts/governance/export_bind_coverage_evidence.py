...
OUTPUT_JSON = Path("docs/en/validation/bind-coverage-evidence.latest.json")
OUTPUT_MD = Path("docs/en/validation/bind-coverage-evidence.latest.md")
...
def _render_markdown(evidence: dict[str, Any]) -> str:
    ...
    for row in evidence["bind_governed_routes"]:
        lines.append(f"- `{row['method']} {row['path']}` (bind_target_metadata_present={row['bind_target_metadata_present']})")
...
def write_bind_coverage_evidence(
    ...
    markdown_path.write_text(_render_markdown(evidence) + "\n", encoding="utf-8")
    return evidence
