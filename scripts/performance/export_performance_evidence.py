...
def _assert_http_status(response: Any, expected_status: int = 200) -> Any:
    """Return response when status matches expected, else raise RuntimeError."""
...
def generate_performance_evidence(
    ...
    if deterministic_fixture:
        ...
        def get_health() -> Any:
            return _assert_http_status(client.get("/v1/health"))

        def get_status() -> Any:
            return _assert_http_status(client.get("/v1/status"))
