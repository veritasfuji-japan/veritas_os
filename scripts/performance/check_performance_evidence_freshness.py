...
EXPECTED_ENVIRONMENT_KEYS: dict[str, type] = {
    "python_version": str,
    "platform": str,
    "implementation": str,
    "ci_detected": bool,
...
def _validate_environment(payload: dict[str, Any], label: str) -> list[str]:
    """Validate required environment object shape and primitive field types."""
    environment = payload.get("environment")
    if not isinstance(environment, dict):
        return [f"{label}: environment must be an object"]
    errors: list[str] = []
    for key, expected_type in EXPECTED_ENVIRONMENT_KEYS.items():
        if key not in environment:
            errors.append(f"{label}: environment missing {key}")
        elif not isinstance(environment[key], expected_type):
            errors.append(
                f"{label}: environment {key} must be {expected_type.__name__}"
            )
    return errors
...
def compare_performance_evidence(
    ...
    if isinstance(committed, dict) and isinstance(generated, dict):
        environment_errors = _validate_environment(committed, str(committed_json))
        environment_errors.extend(_validate_environment(generated, str(committed_json)))
        if environment_errors:
            stale_files.append(str(committed_json))
            reasons.extend(environment_errors)
        ...
        if normalized_committed != normalized_generated:
            stale_files.append(str(committed_json))
            reasons.append(
                f"{committed_json}: content differs from regenerated artifact"
            )
